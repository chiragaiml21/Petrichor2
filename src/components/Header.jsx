    ...
    <>
      ...
      <div className="w-[100%] flex justify-center h-20 border">
        ...
        <div className="header flex justify-between h-20 items-center w-11/12 sm:w-4/5 relative">
            ...
            {menuOpen ? (
              <X strokeWidth={1} className="z-50 text-xl sm:hidden bg-white" onClick={toggleMenu} />
            ) : (
              <Menu strokeWidth={1} className="z-50 text-xl sm:hidden" onClick={toggleMenu} />
            )}
          ...
          <div className="navbar">
            <ul className="flex justify-between">
              ...
              <li className="hidden px-2 hover:scale-110 sm:block">
                <Link to="/login"><User strokeWidth={1} /></Link>
              </li>
              <li className="px-2 hover:scale-110">
                <Link to="cart"><ShoppingCart strokeWidth={1} /></Link>
              </li>
            </ul>
          </div>
        </div>
        ...
        {menuOpen && (
          <div className="z-40 fixed top-0 left-0 w-full h-full bg-white flex flex-col items-center justify-center z-50">
            ...
            <ul className="flex flex-col items-center">
            <li className="px-2 py-4 text-xl hover:underline hover:decoration-solid">
                <Link to="/login" onClick={toggleMenu}><User strokeWidth={1} /></Link>
            </li>
            </ul>
          </div>
        )}
